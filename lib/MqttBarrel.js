...
mqttbarrel.processPresence = async function ( message ) {
	...
	if ( !self.presences[ status.domain ][ status.entity ] ) {
		...
		self.igniteSystemEvent( 'entityAppeared', status.division, status.context || '', status.entity ).catch( self.logger.harconlog )
	}
...
mqttbarrel.intoxicateAnswer = async function ( comm, err, responseComms ) {
	...
	responseComms = error ? [] : responseComms || []
	let packet = JSON.stringify( {
		id: comm.id,
		comm: clean( comm, true ),
		nodeID: self.nodeID,
		err: error,
		response: true,
		responseComms: responseComms.map( (rc) => { return clean( rc, true ) } )
	} )
