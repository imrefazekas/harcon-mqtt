...
mqttbarrel.intoxicateAnswer = async function ( comm, err, responseComms ) {
	...
	self.logger.harconlog( null, 'Appeasing...', {comm: comm, err: err ? err.message || err.toString() : null, responseComms: responseComms}, 'trace' )
	...
	try {
		...
		divisions.forEach( function (division) {
			Object.keys(self.ins[division]).forEach( function (entity) {
				...
				self.client.publish( topicName, JSON.stringify( { division: division, entity: entity, nodeID: self.nodeID } ) )
			} )
		} )
